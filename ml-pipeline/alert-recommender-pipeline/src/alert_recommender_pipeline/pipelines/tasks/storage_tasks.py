...
def save_model(input_model: Input[Model], output_artifact: Output[Artifact]):
    ...
    # Embedded MLServer model implementation to avoid import chain issues at runtime.
    # This must be inside the function because KFP extracts only the function body.
    # Keep in sync with mlserver_model.py if that file changes.
