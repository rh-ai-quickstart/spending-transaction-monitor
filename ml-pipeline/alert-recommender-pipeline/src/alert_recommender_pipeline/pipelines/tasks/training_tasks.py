...
def train_model(input_data: Input[Dataset], output_model: Output[Model]):
    ...
    # Constants (must be defined inside component for KFP serialization)
    ALERT_TYPES = [
        'alert_high_spender',
    ...
    def build_user_features(users_df, transactions_df):
        ...
        # Aggregate transaction statistics per user
    ...
    def generate_heuristic_labels(df):
        """Generate alert labels based on behavioral heuristics when real labels unavailable."""
        df = df.copy()
        
        # Quantile-based alerts (top 25% of users)
        ...
        # Alerts that require real data (default to 0)
    ...
    def generate_labels_from_real_data(user_features, user_alerts_df, alert_types):
        """Generate labels from real user alert preferences."""
        ...
        # Ensure all alert types exist and fill missing values
    ...
    def train_knn_model(X, n_neighbors, metric):
        """Train the KNN model for finding similar users."""
    ...
    def save_model_artifacts(output_path, knn_model, scaler, feature_columns, alert_types, 
        ...
        """Save trained model and related artifacts to pickle file."""
