...
async def get_current_user_profile(
    ...
    try:
        ...
        # Get counts efficiently using database queries instead of loading all data
        from db.models import CreditCard, Transaction

        # Count credit cards efficiently without loading them
        credit_cards_result = await session.execute(
            select(func.count(CreditCard.id)).where(
                CreditCard.user_id == current_user['id']
            )
        )
        credit_cards_count = credit_cards_result.scalar() or 0

        # Count transactions efficiently without loading them
