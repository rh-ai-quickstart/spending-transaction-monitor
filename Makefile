...
PROJECT_NAME ?= spending-transaction-monitor
REGISTRY_URL ?= $(or $(IMAGE_REGISTRY),quay.io)
REPOSITORY   ?= $(or $(IMAGE_REPOSITORY),rh-ai-quickstart)
NAMESPACE    ?= $(or $(OPENSHIFT_NAMESPACE),spending-transaction-monitor)
IMAGE_TAG    ?= $(or $(IMAGE_REPOSITORY_TAG),latest)
IMAGE_PULL_POLICY ?= $(or $(IMAGE_REPOSITORY_PULL_POLICY),Always)
STORAGE_CLASS ?= $(or $(IMAGE_STORAGE_CLASS),"")
