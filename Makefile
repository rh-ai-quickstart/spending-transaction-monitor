...
# Helper function to generate helm parameters for global image settings from environment variables
# Falls back to Makefile defaults if environment variables are not set
define HELM_GLOBAL_PARAMS
$$(if [ -n "$$IMAGE_REGISTRY" ]; then echo "--set global.imageRegistry=$$IMAGE_REGISTRY"; elif [ -n "$(REGISTRY_URL)" ]; then echo "--set global.imageRegistry=$(REGISTRY_URL)"; fi) \
$$(if [ -n "$$IMAGE_REPOSITORY" ]; then echo "--set global.imageRepository=$$IMAGE_REPOSITORY"; elif [ -n "$(REPOSITORY)" ]; then echo "--set global.imageRepository=$(REPOSITORY)"; fi) \
$$(if [ -n "$$IMAGE_REPOSITORY_TAG" ]; then echo "--set global.imageTag=$$IMAGE_REPOSITORY_TAG"; elif [ -n "$(IMAGE_TAG)" ]; then echo "--set global.imageTag=$(IMAGE_TAG)"; fi) \
$$(if [ -n "$$IMAGE_REPOSITORY_PULL_POLICY" ]; then echo "--set global.imagePullPolicy=$$IMAGE_REPOSITORY_PULL_POLICY"; fi) \
$$(if [ -n "$$IMAGE_STORAGE_CLASS" ]; then echo "--set global.storageClass=$$IMAGE_STORAGE_CLASS"; fi)
endef
...
helm-template: helm-dep-update check-env-prod
	...
	export KEYCLOAK_REDIRECT_URIS="$(KEYCLOAK_REDIRECT_URIS_DYNAMIC)"; \
	export KEYCLOAK_WEB_ORIGINS="$(KEYCLOAK_WEB_ORIGINS_DYNAMIC)"; \
	export POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD DATABASE_URL API_KEY BASE_URL LLM_PROVIDER MODEL ENVIRONMENT DEBUG BYPASS_AUTH ALLOWED_HOSTS SMTP_HOST SMTP_PORT SMTP_FROM_EMAIL SMTP_USE_TLS SMTP_USE_SSL KEYCLOAK_REALM KEYCLOAK_CLIENT_ID KEYCLOAK_DB_USER KEYCLOAK_DB_PASSWORD KEYCLOAK_ADMIN_PASSWORD VITE_API_BASE_URL VITE_BYPASS_AUTH VITE_ENVIRONMENT IMAGE_REGISTRY IMAGE_REPOSITORY IMAGE_REPOSITORY_TAG IMAGE_REPOSITORY_PULL_POLICY IMAGE_STORAGE_CLASS; \
