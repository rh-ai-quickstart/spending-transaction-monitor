...
$$(if [ -n "$$SMTP_USERNAME" ]; then echo "--set secrets.SMTP_USERNAME=$$SMTP_USERNAME"; fi) \
$$(if [ -n "$$SMTP_PASSWORD" ]; then echo "--set secrets.SMTP_PASSWORD=$$SMTP_PASSWORD"; fi) \
$$(if [ -n "$$SMTP_FROM_EMAIL" ]; then echo "--set secrets.SMTP_FROM_EMAIL=$$SMTP_FROM_EMAIL"; fi) \
$$(if [ -n "$$SMTP_REPLY_TO_EMAIL" ]; then echo "--set secrets.SMTP_REPLY_TO_EMAIL=$$SMTP_REPLY_TO_EMAIL"; fi) \
...
$$(if [ -n "$$TWILIO_ACCOUNT_SID" ]; then echo "--set secrets.TWILIO_ACCOUNT_SID=$$TWILIO_ACCOUNT_SID"; fi) \
$$(if [ -n "$$TWILIO_AUTH_TOKEN" ]; then echo "--set secrets.TWILIO_AUTH_TOKEN=$$TWILIO_AUTH_TOKEN"; fi) \
$$(if [ -n "$$TWILIO_PHONE_NUMBER" ]; then echo "--set secrets.TWILIO_PHONE_NUMBER=$$TWILIO_PHONE_NUMBER"; fi) \
...
deploy: create-project helm-dep-update check-keycloak-vars
	...
	export POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD DATABASE_URL API_KEY BASE_URL LLM_PROVIDER MODEL MODEL_ID MODEL_URL MODEL_API_KEY ENVIRONMENT DEBUG BYPASS_AUTH ALLOWED_HOSTS SMTP_HOST SMTP_PORT SMTP_USERNAME SMTP_PASSWORD SMTP_FROM_EMAIL SMTP_REPLY_TO_EMAIL SMTP_USE_TLS SMTP_USE_SSL TWILIO_ACCOUNT_SID TWILIO_AUTH_TOKEN TWILIO_PHONE_NUMBER KEYCLOAK_REALM KEYCLOAK_CLIENT_ID KEYCLOAK_DB_USER KEYCLOAK_DB_PASSWORD KEYCLOAK_ADMIN_PASSWORD VITE_API_BASE_URL VITE_BYPASS_AUTH VITE_ENVIRONMENT LLAMASTACK_BASE_URL LLAMASTACK_MODEL USE_ML_RECOMMENDATIONS USE_ML_INFERENCE_SERVICE ML_INFERENCE_ENDPOINT; \
